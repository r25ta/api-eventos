...
public class EventoDAOImpl extends JdbcDaoSupport implements EventoDAO {
	...
	public List<EventoModel> getByCtlPlvia(Long ctlPlvia) {
		...
		try{
			...
			throw new ViagemInvalidaException("Erro na execução! " + SQL_GET_EVENTOS + WHERE_BY_CTL_PLVIA + ORDER_BY + " Causa do Erro -> "+ e);
	...
	public List<EventoModel> getByDocumentoProprio(String codDocum){
		...
		try{
			...
			throw new ViagemInvalidaException("Erro na execução! " + SQL_GET_EVENTOS + WHERE_BY_COD_DOCUM_FIS + ORDER_BY + " Causa do Erro -> "+ e);
