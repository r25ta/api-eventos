...
import br.com.pamcary.api.evento.exception.ViagemNotFoundException;
...
public class ViagemDAOImpl extends JdbcDaoSupport implements ViagemDAO {
	...
	private static final String SQL_QUERY_PLANO_VIAGEM = " select " + " pv.ctl_plvia " 
			...
			+ " ,pv.dhr_Posic_Ult " + " ,pv.des_Praca_Ult " + " ,pv.num_Latit_Ult " + " ,pv.num_Longi_Ult "
			+ " ,pv.dhr_Inclu " + " ,pv.nom_User_Inc " + " ,pv.tip_Rastr "
			...
			+ " where tip_opera_vie = pv.tip_opera_vie) as des_opera_vie " + " ,pv.dhr_Atvie_Pln "
			+ " ,pv.cod_Atvie_Usu " + " ,pv.dhr_Posic_Prm " + " ,pv.num_Latit_Prm ,pv.num_Longi_Prm "
	...
	private static final String FROM_PLANO_VIAGEM_WHERE_BY_DOCUMENTO_PROPRIO = " from plano_viagem pv "
			...
			+ " where pv.ctl_plvia = rpd.ctl_plvia " +  "  and dfc.tip_docum_fis = 7 " + "  and rpd.ord_desti = 0 "
	...
	private void initialize() {
		setDataSource(oracleDataSource.oracleDataSource());
	}
	...
	public List<ViagemModel> getByCtlPLvia(long ctlPlvia) {
		...
		try{
			...
			throw new ViagemNotFoundException("Erro na execução! " + SQL_QUERY_PLANO_VIAGEM + FROM_PLANO_VIAGEM_WHERE_BY_CTL_PLVIA + FILTRO_TIPO_OPERACAO +" Causa do Erro -> ",e);
	...
	public List<ViagemModel> getByCodDocumentoProprio(String codDocum) {
		...
		try {
			...
			throw new ViagemNotFoundException("Erro na execução! " + SQL_QUERY_PLANO_VIAGEM + FROM_PLANO_VIAGEM_WHERE_BY_DOCUMENTO_PROPRIO + FILTRO_TIPO_OPERACAO + " Causa do Erro -> ",e);
